...
- (void)AnalyticalData:(NSDictionary *)dic {
    ...
    NSLog(@"AnalyticalData%@  dic%@", urseDataDic, dic);
    if ([state isEqualToString:@"1"]) {
        [self AnalysisUserData:urseDataDic[@"memberId"]];
    }else {
        UIAlertController *alertV = [UIAlertController alertControllerWithTitle:@"抱歉!" message:[NSString stringWithFormat:@"登录失败,%@", dic[@"result"]] preferredStyle:UIAlertControllerStyleAlert];
