...
#import "ShopCalculateFreightVC.h"
#import "ShopOrderHeadTVCell.h"
#import "OrderGoodsTVCell.h"//订单里面的商品展示
#import "CalculateFreightTVCell.h"

#import "OrderCFVC.h"//计算运费界面

@interface ShopCalculateFreightVC ()<UITableViewDelegate, UITableViewDataSource, CalculateFreightTVCellDelegate>
...
@implementation ShopCalculateFreightVC
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"待商家计算运费";
...
- (void)creatTableView {
    ...
    [_tableView registerClass:[CalculateFreightTVCell class] forCellReuseIdentifier:@"CalculateFreightTVCell"];
...
- (void)RequestOrderListData {
    ...
    NSDictionary *url_Dic = @{@"memberId":[UserDataSingleton mainSingleton].userID,@"pageNo":@"0",@"pageSize":@"10",@"orderState":@"5",@"storeId":[UserDataSingleton mainSingleton].storeID, @"buyingPatternsName":@""};
    ...
    __block ShopCalculateFreightVC *VC = self;
...
- (void)AnalyticalSellerOrderListData:(NSString *)dataStr {
    ...
    if (tempDictQueryDiamond.count == 0) {
        [self showAlert:@"暂时没有订单"];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        CalculateFreightTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CalculateFreightTVCell" forIndexPath:indexPath];
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.model = goodsArr[indexPath.row - 1];
        return cell;
    }
}
...
- (void)CalculateFreight:(OrderBtn *)sender {
    ...
    NSLog(@"%ldCalculateFreightd", (long)sender.indexPath.section);
    SellerOrderListModel * model = self.DataArray[sender.indexPath.section];
    NSDictionary *addressDic = (NSDictionary *)model.address;
    ...
    //根据描述 创建实体对象
    ShippingAddressModel *OrderAddressModel = [[ShippingAddressModel alloc] initWithEntity:des insertIntoManagedObjectContext:self.managedContext];
    for (NSString *addressKey in addressDic) {
        [OrderAddressModel setValue:addressDic[addressKey] forKey:addressKey];
    }
    OrderCFVC *VC = [[OrderCFVC alloc] init];
    VC.buyersModel = OrderAddressModel;
    VC.OrderSn = model.orderSn;
    NSLog(@"OrderAddressModel%@",OrderAddressModel);
    [self.navigationController pushViewController:VC animated:YES];
